...
class BindActionHandler[T <: EvalContext](element: String, event: ElementEvent.Value, javascript: String, engine: EvalEngine[WebContext], ctx: WebContext) extends UnitStep[WebContext](engine, ctx) {
  ...
  override def apply(parent: Identifiable, step: Step): Unit = {
    ...
    env.scopes.set(JavaScriptBinding.key(s"$element/action/${ElementEvent.actionOf(event)}"), javascript)
  }
