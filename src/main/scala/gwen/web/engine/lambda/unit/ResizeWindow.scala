...
class ResizeWindow[T <: EvalContext](width: Int, height: Int, engine: EvalEngine[WebContext], ctx: WebContext) extends UnitStep[WebContext](engine, ctx) {

  override def apply(parent: Identifiable, step: Step): Unit = {
    engine.checkStepRules(step, BehaviorType.Action, env)
    ctx.resizeWindow(width, height)
  }
