...
class WebEngine extends EvalEngine[WebContext] {
  ...
  override def translateStep(step: Step): UnitStep[WebContext] = {
    step.expression match {
      ...
      case r"""(.+?)$element( text| value)?$selection should( not)?$negation (be|contain|start with|end with|match regex|match xpath|match json path)$operator (.+?)$attribute""" if attribute != "absent" && !element.matches(".+at (json path|xpath).+") && !attribute.contains("% similar to ") && !attribute.contains('"') =>
