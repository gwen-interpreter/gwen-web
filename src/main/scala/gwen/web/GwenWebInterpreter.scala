...
object GwenWebInterpreter extends GwenInterpreter(new WebEngine()) with WebProjectInitialiser {
  ...
  override def initDefaultSettings(options: GwenOptions): GwenOptions = {
    ...
    val opts = if (!options.init) {
      ...
      val browserConf = {
        if (WebBrowser.findSettingsFile(options.settingsFiles).isEmpty) {
      ...
      val envConf = {
        if (options.settingsFiles.filter(_.getPath.startsWith(s"$baseDir${File.separatorChar}env${File.separatorChar}")).isEmpty) {
