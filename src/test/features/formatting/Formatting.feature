  ...
  Scenario: I format a date
    Given ISO date is "2026-03-14"
     ...
     Then dmy date should be "14/03/2026"
      And dmy date should match datetime format "dd/MM/yyyy"
  ...
  Scenario: I format an ISO date to a display date with oridnal day suffix
    Given ISO date is "2026-03-02"
     ...
     Then display date should be "2nd March 2026"
      And display date should match datetime format "d(st|nd|rd|th) MMMM yyyy"
  ...
  Scenario: I format a display date with oridnal day suffix to an ISO date
    Given display date is "23rd March 2026"
     ...
     Then ISO date should be "2026-03-23"
      And ISO date should match datetime format "yyyy-MM-dd"
  ...
  Scenario: I format a number
    Given number is "1234"
     ...
     Then dollars should be "$1,234.00"
      And dollars should match number format "$#,##0.00"

  Scenario: I format a number to a number with an ordinal suffix
    Given number is "22"
     When @Number I format number from "#" to "#(st|nd|rd|th)" as original number
     Then original number should be "22nd"
      And original number should match number format "#(st|nd|rd|th)"

  Scenario: I format a number with an ordinal suffix to a leading zero number
    Given original number is "1st"
     When @Number I format original number from "#(st|nd|rd|th)" to "00" as leading zero number
     Then leading zero number should be "01"
      And leading zero number should match number format "00"
  ...
  Scenario: I capture the current date and time
    Given day is "${gwen.now:E}"
     ...
     Then day should be "${day name}"
      ...
      And day should match datetime format "E"
