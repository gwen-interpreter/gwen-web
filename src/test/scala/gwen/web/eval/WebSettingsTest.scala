...
class WebSettingsTest extends BaseTest with Matchers with MockitoSugar {
  ...
  "Default Gwen web settings" should "should load" in {
    Settings.exclusively {
      ...
      WebSettings.`gwen.web.browser.position` should be (None)
      ...
      WebSettings.`gwen.web.highlight.style` should be ("background: yellow; border: 1px solid gold;")
      ...
      WebSettings.`gwen.web.remote.connectTimeout.seconds` should be (60L)
      ...
      WebSettings.`gwen.web.session.expired.autoReplace` should be (true)
