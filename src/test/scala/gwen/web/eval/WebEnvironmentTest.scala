...
class WebEnvironmentTest extends BaseTest with Matchers with MockitoSugar {
  ...
  "Timeout on compare" should "result in assertion error" in {
    ...
    intercept[AssertionError] {
      ctx.compare("a", "2", () => "1", ComparisonOperator.be, negate = false, mask = false, None, None, AssertionMode.hard)
    }
  }
  ...
  "Timeout on negated compare" should "result in assertion error" in {
    val ctx = newCtx()
    intercept[AssertionError] {
      ctx.compare("a", "2", () => "2", ComparisonOperator.be, negate = true, mask = false, None, None, AssertionMode.hard)
    }
  }
