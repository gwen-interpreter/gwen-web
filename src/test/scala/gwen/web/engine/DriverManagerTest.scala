...
import gwen.web._
import gwen.web.engine.WebErrors.NoSuchWindowException
...
class DriverManagerTest extends BaseTest with Matchers with MockitoSugar with BeforeAndAfterEach {
  ...
  private def newDriverManager(): DriverManager ={
    new DriverManager {
      override private [engine] def chrome(): WebDriver = createMockLocalDriver() tap { driver =>
        mockChromeDriver = driver
      }
      override private [engine] def firefox(): WebDriver = createMockLocalDriver() tap { driver =>
        mockFirefoxDriver = driver
      }
      override private [engine] def ie(): WebDriver = createMockLocalDriver() tap { driver =>
        mockIeDriver = driver
      }
      override private [engine] def edge(): WebDriver = createMockLocalDriver() tap { driver => 
        mockEdgeDriver = driver
      }
      override private [engine] def safari(): WebDriver = createMockLocalDriver() tap { driver =>
        mockSafariDriver = driver
      }
      override private [engine] def remote(hubUrl: String, capabilities: DesiredCapabilities): WebDriver = {
  ...
  private def newDriverManager(mockDriver: WebDriver): DriverManager = new DriverManager() { 
    override private [engine] val drivers = mutable.Map() += ("primary" -> mockDriver)
  }
