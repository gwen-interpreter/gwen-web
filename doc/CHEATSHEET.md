...
| I capture the `text|node|nodeset` in `<element|attribute|property>` by `xpath|regex` "`<expression>`" as `<attribute>` | Extracts and binds a value from an element, attribute, or property setting into an attribute |`<element|attribute|property>` = the element, attribute, or property setting to extract the value from, `<expression>` = the extractor expression, `<attribute>` = the attribute to store the captured value into |
