...
| `<target>` `is|will be` `<source>` | Binds a named source attribute to a named target attribute | `<target>` = the name of the attribute to bind the value to, `<source>` = the attribute containing the value to bind |
